/**
 * Created by weiqiujuan on 17-9-12.
 */

function getDB() {
    var db = window.openDatabase("student", "1.0", "student manager system db", 2 * 1024 * 1024);
    return db ;
}
//创建数据库表
(function () {
...
(function () {
    var dbt = getDB();
    dbt.transaction(function (transaction) {
        var sql = "CREATE TABLE IF NOT EXISTS " +
            "tbl_user(" +
            "name TEXT," +
            "password int)";
...
// 保存用户信息
function saves(User,handler){
    console.log('running saves')
    if(User.name&&User.password){
        var dbt=getDB();
        dbt.transaction(function(transaction){
            var sql="insert into tbl_user values(?,?)";
            transaction.executeSql(sql,[User.name,User.password],function () {
                handler.call(this);
            });
        });
    }else{
